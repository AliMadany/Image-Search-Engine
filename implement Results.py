...
def find_similar_images(input_image_features, dataset_features, dataset_filepaths, similarity_threshold=0.5, max_display=10, max_per_window=6):
    ...
    for start_idx in range(0, num_similar_images, max_per_window):
        ...
        cols = 3
        rows = (current_batch_size + cols - 1) // cols  # Ensure enough rows to fit all images

        # Create a new figure for the current batch of images
...
max_display = 15  # Maximum number of similar images to display
...
find_similar_images(input_image_features, dataset_features, dataset_filepaths, similarity_threshold, max_display, max_per_window)
